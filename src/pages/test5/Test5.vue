<template>
    <div class="todo-wrapper page">
        <div class="input-wrapper input-or-btn">
            <input type="text" placeholder="Please input content ..." v-model="editContent">
            <button @click="addAction">Add</button>
        </div>
        <div v-if="hasList" class="content-wrapper">
            <div class="input-or-btn">
                <input type="text" placeholder="Please input keywords ..." v-model="searchWord">
                <!-- <button @click="searchAction">查找</button> -->
            </div>
            <ul class="list-content">
                <li v-for="item in showList" :key="item.id" class="input-or-btn">
                    <span :class="{'text-content': item.finished}">{{item.text}}</span>
        ...
        <div v-if="hasList" class="menu-wrapper">
            <button v-for="item in btnState" :key="item" @click="checkStatus(item)" :class="{'btnBg': item === currentState}">{{getStateName(item)}}</button>
        </div>
        ...
        <div v-if="showMask" class="mask" @click.self="closeMask">
            <div class="edit-dialog">
                ...
                <textarea class="text-area" placeholder="Please input content ..." v-model="editCon"></textarea>
                <div class="btn">
...
export default {
    ...
    computed: {
        ...
        showList(){ //【有‘查找’按钮时此计算属性注释掉，不需要用】
            ...
            return this.searchWord ? this.searchInfoFromObjectArr(this.searchWord, tempList) : tempList
        },
        hasList(){
    ...
    methods: {
        ...
        addAction(){
            ...
            this.todoList.push({id: this.id, text: this.editContent, finished: false})
            this.editContent = ''
            this.id++
        ...
        finishAction(id){
            const index = this.findIdFromObjectArr(id, this.todoList)
            this.todoList.splice(index, 1, {...this.todoList[index], finished: true})
            // this.showList = this.todoList // 【有‘查找’按钮时需要解封此行代码】
        },
        // 编辑待办事件
        editAction(item){
...
.todo-wrapper{
    .input-wrapper, .content-wrapper, .menu-wrapper, .tip-content{
        padding: 10px;
